...
from .models import Response, EmailVerification, User, categories, Letter
...
def home(request):
    ...
    if request.user.is_authenticated:
        if request.method == 'POST':
            if request.POST['letter'] != '':
                ...
                if id == '':
                    ...
                    letter = Letter.objects.filter(user=request.user).first()
                    letter.letter = request.POST['letter']
                letter.save()
                # messages.success(request, extra_tags='alert alert-success', message='Your letter has been saved')
                # return json of letter
                return Letter.objects.filter(id=letter.id).first()
        ...
        myfields = Response.objects.filter(user=request.user)
        # get letter by user one or none
        letter = Letter.objects.filter(user=request.user).first()
